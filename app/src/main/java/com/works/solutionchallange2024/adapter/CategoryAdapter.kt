...
class CategoryAdapter (var mContext: Context, var categoryListesi:List<Category>):
    ...
    inner class CategoryDesignHolder(var design:MenuCategoryBinding):RecyclerView.ViewHolder(design.root)
    ...
    override fun onBindViewHolder(holder: CategoryDesignHolder, position: Int) {
        ...
        t.categoryCardView.setOnClickListener {
            //Snackbar.make(it,"${category.categoryName}",Snackbar.LENGTH_SHORT).show()
            val gecis= CategoryFragmentDirections.actionCategoryFragmentToHomePageFragment(category.categoryName)
